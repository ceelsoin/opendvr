...
- **Auto-captioning** (Settings page, global): for notable object-detection events, generates a short caption per category (person/vehicle/animal/other), independently toggleable. Two provider modes: **External** (any OpenAI-compatible vision endpoint - hosted API, remote Ollama/LM Studio, or the optional GPU-accelerated `llamacpp-gpu` docker-compose service) or **Local** (the backend itself spawns/manages a CPU-only llama.cpp process in-container, running the bundled SmolVLM-500M GGUF model - works out of the box, nothing to download). The caption is stored on the event and appended to external notifications. See [Configuration → AI computer vision](./configuration.md#ai-computer-vision-object-detection-face-recognition-auto-captioning) for setup and the GPU option.
